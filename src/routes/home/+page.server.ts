...
//This will be called when home page load.
...
export const load = (async ({ cookies }) => {
    ...
    if (hashValue) {
        ...
        if (result) {
            return {homeAuth: true, currentUser: JSON.stringify(result[0])};
        }
    }
    return {homeAuth: false, currentUser: JSON.stringify({status: "error"})};
});